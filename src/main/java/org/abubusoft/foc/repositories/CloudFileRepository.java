...
//	
	...
	//
	@Query("SELECT f FROM CloudFile f JOIN f.consumer c JOIN f.uploader u WHERE c.id=:consumerId and u.id=:uploaderId ORDER BY f.viewed ASC, f.createdDateTime DESC ")		
	List<CloudFile> findByUploaderAndConsumer(long uploaderId, long consumerId);

	//
	@Query("SELECT f FROM CloudFile f JOIN f.consumer c JOIN f.uploader u JOIN f.tags t WHERE u.id=:uploaderId AND c.id=:consumerId AND t IN :tags ORDER BY f.viewed ASC, f.createdDateTime DESC")
